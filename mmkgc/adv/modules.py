...
class BaseGenerator(nn.Module):
    def __init__(
        ...
        self.proj_dim = (structure_dim + img_dim) // 2
        self.noise_dim = int(structure_dim * 0.125)
        self.generator_model = nn.Sequential(
            nn.Linear(self.noise_dim + structure_dim, self.proj_dim),
            nn.LeakyReLU(),
...
class RelGenerator(nn.Module):
    def __init__(
        ...
        self.rel_generator = BaseGenerator(noise_dim, structure_dim * 9, structure_dim)  # rv
    def forward(self, rs, hv, ht, tv, tt):
        rel_joint_emb = torch.cat((rs, hv, ht, tv, tt), dim=-1)
        return self.rel_generator(rel_joint_emb)
...
class MultiGenerator(nn.Module):
    def __init__(
        ...
        self.ent_joint_generator = BaseGenerator(noise_dim, structure_dim, img_dim)  # rv
        self.rel_generator = BaseGenerator(noise_dim, structure_dim // 2, img_dim // 2)  # rv
    def forward(self, batch_ent_emb, mode = 'e'):
        # if modal == 1:
        ...
        if mode == 'e':
            return self.ent_joint_generator(batch_ent_emb)
        elif mode == 'r':
            return self.rel_generator(batch_ent_emb)
