...
class AdvRelRotatE(Model):
    ...
    def get_joint_embeddings(self, es, ev, et, rg):
        e = torch.stack((ev, et), dim=1) ######################## es
#        e = ev.unsqueeze(1)
        u = torch.tanh(e)         
        scores = self.ent_attn(u).squeeze(-1)         
        attention_weights = torch.softmax(scores, dim=-1)  # Design of V8 
        rg = torch.sigmoid(rg)    
        context_vectors = rg * es + (1 - rg) * torch.sum(attention_weights.unsqueeze(-1) * e, dim=1)         
    ...
    def _calc(self, h, t, r, mode): #RotateE
        pi = self.pi_const
